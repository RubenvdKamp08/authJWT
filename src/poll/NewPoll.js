...
class NewPoll extends Component {
    ...
    handleSubmit(event) {
        ...
        const pollData = {
            projectName: this.state.question.text,
            tasks: this.state.choices.map(choice => {
                return {text: choice.text} 
            }),
            projectLength: this.state.pollLength
        };
                    ...
                    <Form onSubmit={this.handleSubmit} className="create-poll-form">
                        ...
                        <FormItem className="poll-form-row">
                            <Button type="dashed" onClick={this.addChoice} disabled={this.state.choices.length === MAX_CHOICES}>
                                <Icon type="plus" /> Add an extra task
                            </Button>
                        </FormItem>
                        <FormItem className="poll-form-row">
                            <Col xs={24} sm={4}>
...
function PollChoice(props) {
    return (
        ...
        help={props.choice.errorMsg} className="poll-form-row">
            <Input 
                placeholder = {'Task ' + (props.choiceNumber + 1)}
                size="large"
